...
```ts
type LengthOfStringRecursive<S extends string> = S extends `${infer S1}${infer S2}` ? S1 extends "" ? [] : [S1, ...LengthOfStringRecursive<S2>] : [];
type LengthOfString<S extends string> = S extends `${infer S1}${infer S2}` ? S1 extends "" ? [] : [S1, ...LengthOfStringRecursive<S2>]["length"] : 0;
```
