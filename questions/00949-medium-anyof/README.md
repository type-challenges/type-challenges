...
Implement a type-level AnyOf<T> that returns `true` if a tuple contains at least one non-empty type, otherwise `false`. Emptiness is defined statically (`false`, 0, '', [], {}), independent of JavaScript runtime behavior.
