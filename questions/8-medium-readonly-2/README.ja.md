...
2つの型引数`T`と`K`を取る汎用`MyReadonly2<T, K>`を実装します。

`K`は、読み取り専用に設定する必要がある`T`の場合、プロパティのセットを指定します。 `K`が指定されていない場合は、通常の`Readonly<T>`と同様に、すべてのプロパティを読み取り専用にする必要があります。
